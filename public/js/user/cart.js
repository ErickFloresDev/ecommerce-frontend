...
function renderCart() {
  ...
  summary.innerHTML = `
    <div class="summary-card">
      ...
      <a href="/user" class="btn btn-outline-dark-buy mt-3">
...
//FUNCION PARA FINALIZAR LA COMPRAR
async function finalizePurchase() {
  const token = localStorage.getItem('token');
  if (!token) {
    return showNotification('Debes iniciar sesión para continuar');
  }

  // 1. Recupera el carrito y calcula total y detalles
  const cart = JSON.parse(localStorage.getItem('cart')) || [];
  if (cart.length === 0) {
    return showNotification('El carrito está vacío');
  }

  let total = 0;
  const order_details = cart.map(item => {
    const subtotal = Number((item.price_end * item.quantity).toFixed(2));
    total += subtotal;
    return {
      id_product: item.id_product,
      amount: item.quantity,
      unit_price: item.price_end,
      subtotal
    };
  });
  ...
  try {
    ...
    const res = await fetch(`${CONFIG.API_URL}/orders`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${token}`  // si tu API espera JWT
      },
